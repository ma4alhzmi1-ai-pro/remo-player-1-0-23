...
class ExpoVideoPlaybackService : MediaSessionService() {
  ...
  private val commandNext = SessionCommand(NEXT_COMMAND, Bundle.EMPTY)
  private val commandPrevious = SessionCommand(PREVIOUS_COMMAND, Bundle.EMPTY)
  private val nextButton = CommandButton.Builder()
    .setDisplayName("next")
    .setSessionCommand(commandNext)
    .setIconResId(R.drawable.next_track)
    .build()

  private val previousButton = CommandButton.Builder()
    .setDisplayName("previous")
    .setSessionCommand(commandPrevious)
    .setIconResId(R.drawable.previous_track)
    .build()
  ...
  fun setShowNotification(showNotification: Boolean, player: ExoPlayer) {
    appContext.mainQueue.launch {
      ...
      mediaSessions[player]?.let {
        ...
        if (showNotification) {
          onUpdateNotification(it, player.playWhenReady)
        } else {
  ...
  fun registerPlayer(videoPlayer: VideoPlayer) {
    appContext.mainQueue.launch {
      ...
      val mediaSession = MediaSession.Builder(this@ExpoVideoPlaybackService, player)
        ...
        .setCallback(VideoMediaSessionCallback { action ->
          appContext.mainQueue.launch {
            when (action) {
              NEXT_COMMAND -> videoPlayer.emit("remoteNext", emptyMap<String, Any>())
              PREVIOUS_COMMAND -> videoPlayer.emit("remotePrevious", emptyMap<String, Any>())
            }
          }
        })
        .setCustomLayout(ImmutableList.of(previousButton, seekBackwardButton, seekForwardButton, nextButton))
        .build()
  ...
  companion object {
    ...
    const val NEXT_COMMAND = "NEXT_TRACK"
    const val PREVIOUS_COMMAND = "PREVIOUS_TRACK"
