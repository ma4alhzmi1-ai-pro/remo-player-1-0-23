...
import { Alert, Linking, Pressable, ScrollView, StyleSheet, Switch, Text } from "react-native";
...
import { APP_VERSION, RELEASES_URL } from "@/lib/app-release";
...
export default function SettingsScreen() {
  ...
  return <ScreenContainer className="px-5"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} onScrollBeginDrag={refresh}>
    ...
    <SectionTitle title="الخصوصية والحافظة" /><Surface style={styles.list}><SettingRow icon="content-paste" title="الحافظة المحلية" subtitle={`الحد الحالي: ${settings?.clipboardLimit ?? 30} عنصرًا. النص الطويل لا يُقصَّر.`} /><SettingRow icon="mic" title="تحويل الصوت إلى نص" subtitle="يستخدم إذن الميكروفون ومحرك التعرف الصوتي المتاح في أندرويد." /><SettingRow icon="privacy-tip" title="سياسة الخصوصية" subtitle="كيف نتعامل مع النصوص والحافظة والصوت." onPress={() => router.push("/privacy" as never)} /></Surface>
    <SectionTitle title="حول التطبيق" /><Surface style={styles.list}><SettingRow icon="info-outline" title="إصدار التطبيق" subtitle={`ريموكيبورد مزخرف ${APP_VERSION}`} /><SettingRow icon="system-update" title="التحقق من التحديث" subtitle="افتح صفحة الإصدارات على GitHub لتنزيل أحدث APK." onPress={() => Linking.openURL(RELEASES_URL)} /><SettingRow icon="person-outline" title="عن المطور" subtitle="محمد الحزمي وروابط التواصل والمنصات." onPress={() => router.push("/about" as never)} /></Surface>
