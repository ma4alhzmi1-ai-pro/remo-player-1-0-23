...
export default function SettingsScreen() {
  ...
  const handleCleanCache = async () => {
    const before = await getCacheSizeInfo();
    const result = await cleanOrphanedCacheFiles();
    const freedMB = (result.freedBytes / (1024 * 1024)).toFixed(2);
    Alert.alert(
      "تنظيف الذاكرة والتخزين",
      `تم فحص التخزين المؤقت وحذف ${result.deletedCount} ملفاً مكرراً/مؤقتاً.\nتم توفير ${freedMB} ميجابايت من مساحة الجهاز.`
    );
  };
