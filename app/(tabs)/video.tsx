...
import { Artwork, colors, EmptyState, formatDuration, MediaRow, ActivePlayingBadge } from "@/components/remo-ui";
...
export default function VideoScreen() {
  ...
  return <ScreenContainer className="px-0">
    <FlatList
      ...
      renderItem={({ item }) => {
        if (view === "grid") {
          const isCurrent = Boolean(currentItem && (currentItem.id === (item as MediaItem).id || currentItem.uri === (item as MediaItem).uri));
          return <VideoCard item={item as MediaItem} isCurrent={isCurrent} isPlaying={isPlaying} onPress={() => void openVideo(item as MediaItem)} onAddToPlaylist={() => setPlaylistTarget(item as MediaItem)} />;
        }
        if (view === "folders") {
          const folder = item as unknown as Folder;
          const isCurrent = Boolean(currentItem && folder.items.some((it) => it.id === currentItem.id || it.uri === currentItem.uri));
          return <FolderRow folder={folder} isCurrent={isCurrent} onPress={() => setSelectedFolder(folder)} />;
        }
        const pl = item as unknown as VideoPlaylist;
        const isCurrent = Boolean(currentItem && pl.items.some((it) => it.id === currentItem.id || it.uri === currentItem.uri));
        return <PlaylistRow playlist={pl} isCurrent={isCurrent} onPress={() => setSelectedPlaylist(pl)} />;
      }}
      ListHeaderComponent={<>
...
function VideoCard({ item, isCurrent = false, isPlaying = false, onPress, onAddToPlaylist }: { item: MediaItem; isCurrent?: boolean; isPlaying?: boolean; onPress: () => void; onAddToPlaylist: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.videoCard, isCurrent && styles.videoCardActive, pressed && styles.pressed]}>
      <View style={[styles.videoPreview, isCurrent && styles.videoPreviewActive]}>
        {item.thumbnailUri ? <Image source={{ uri: item.thumbnailUri }} style={styles.thumbnail} /> : <Artwork item={item} size={60} />}
        <View style={styles.playShade}>
          <MaterialIcons name={isCurrent && isPlaying ? "pause-circle-filled" : "play-circle-filled"} size={37} color="rgba(255,255,255,0.92)" />
        </View>
        <View style={styles.durationBadge}>
          <Text style={styles.durationText}>{item.duration ? formatDuration(item.duration) : "فيديو"}</Text>
        </View>
        {isCurrent && (
          <View style={styles.cardActiveIndicator}>
            <MaterialIcons name="graphic-eq" size={12} color="#08111F" />
            <Text style={styles.cardActiveText}>مشغّل</Text>
          </View>
        )}
        <Pressable onPress={(event) => { event.stopPropagation(); onAddToPlaylist(); }} style={styles.addToPlaylist} accessibilityLabel="إضافة إلى قائمة تشغيل">
          <MaterialIcons name="playlist-add" size={19} color={colors.text} />
        </Pressable>
      </View>
      <Text numberOfLines={2} style={[styles.videoTitle, isCurrent && styles.videoTitleActive]}>{item.title}</Text>
      <Text numberOfLines={1} style={styles.videoMeta}>{getMediaFolderName(getMediaFolderPath(item.uri))}</Text>
    </Pressable>
  );
}

function FolderRow({ folder, isCurrent = false, onPress }: { folder: Folder; isCurrent?: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.collectionRow, isCurrent && styles.collectionRowActive, pressed && styles.pressed]}>
      <View style={[styles.collectionIcon, isCurrent && styles.collectionIconActive]}>
        <MaterialIcons name={isCurrent ? "folder-special" : "folder"} size={28} color={isCurrent ? colors.cyan : "#B7C0CC"} />
      </View>
      <View style={styles.collectionCopy}>
        <View style={styles.collectionTitleRow}>
          {isCurrent && (
            <View style={styles.miniActiveDot}>
              <MaterialIcons name="graphic-eq" size={12} color={colors.cyan} />
            </View>
          )}
          <Text numberOfLines={1} style={[styles.collectionTitle, isCurrent && styles.collectionTitleActive]}>
            {folder.name}
          </Text>
        </View>
        <Text numberOfLines={1} style={styles.collectionMeta}>{folder.items.length} مقاطع فيديو · {folder.path}</Text>
      </View>
      {isCurrent ? <ActivePlayingBadge label="مشغّل هنا" compact /> : <MaterialIcons name="chevron-left" size={23} color={colors.muted} />}
    </Pressable>
  );
}

function PlaylistRow({ playlist, isCurrent = false, onPress }: { playlist: VideoPlaylist; isCurrent?: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.collectionRow, isCurrent && styles.collectionRowActive, pressed && styles.pressed]}>
      <View style={[styles.collectionIcon, styles.playlistIcon, isCurrent && styles.collectionIconActive]}>
        <MaterialIcons name="queue-music" size={26} color={colors.cyan} />
      </View>
      <View style={styles.collectionCopy}>
        <View style={styles.collectionTitleRow}>
          {isCurrent && (
        ...
        <Text style={styles.collectionMeta}>{playlist.items.length} مقاطع فيديو</Text>
      </View>
      {isCurrent ? <ActivePlayingBadge label="مشغّل هنا" compact /> : <MaterialIcons name="chevron-left" size={23} color={colors.muted} />}
    </Pressable>
  );
...
const styles = StyleSheet.create({
  ...
  collectionRowActive: {
    backgroundColor: "rgba(46, 197, 255, 0.08)",
    borderRightWidth: 3,
    borderRightColor: colors.cyan,
  },
  collectionIconActive: {
    backgroundColor: "rgba(46, 197, 255, 0.2)",
  },
  collectionTitleRow: {
    flexDirection: "row-reverse",
    alignItems: "center",
