...
const views: Array<{ id: VideoView; label: string; icon: keyof typeof MaterialIcons.glyphMap }> = [
...
export default function VideoScreen() {
  ...
  return <ScreenContainer className="px-0">
    <FlatList
      key={`video-${view}`}
      data={(view === "grid" ? matchingVideos : view === "folders" ? folders : videoPlaylists) as Array<MediaItem | Folder | VideoPlaylist>}
...
function getFolderName(uri: string) { const parts = decodeURIComponent(uri).split("/").filter(Boolean); const candidate = parts.length > 1 ? parts[parts.length - 2] : "الفيديوهات"; return candidate.replace(/[-_]+/g, " ") || "الفيديوهات"; }
function makeFolders(videos: MediaItem[]): Folder[] { return groupMediaFolders(videos); }
